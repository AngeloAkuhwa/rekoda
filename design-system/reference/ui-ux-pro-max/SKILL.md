> **Vendored into this repo 19 Aug 2026 — read this before any UI work.**
>
> This is the `ui-ux-pro-max` skill's **instruction layer**, committed so that
> every session has the rules even when the skill itself is not synced.
>
> **What this file is not:** it is not the skill's _dataset_. The 79 styles, 192
> product palettes, 74 font pairings, 119 UX guidelines, 105 icons, 17 GSAP
> presets, 25 chart types and 22 stacks live in separate files alongside
> `scripts/search.py`, and **those do not sync to remote Claude Code sessions**
> (verified 19 Aug 2026 — only `SKILL.md` is present). So `--design-system`,
> `--domain` and `--stack` searches **cannot run remotely**.
>
> **How to use it here:**
>
> - **Locally**, with the full skill installed: run the searches per §Workflow,
>   then reconcile the output into `design-system/rekoda/MASTER.md`.
> - **Remotely**, or whenever a search returns nothing: fall back to the
>   **Rule Categories by Priority** table below (P1 accessibility → P10 charts)
>   and, per the skill's own instruction, **say explicitly that the result came
>   from built-in defaults rather than a database match.**
> - Either way, `design-system/rekoda/MASTER.md` is the binding artefact for
>   Rekoda, and `docs/design-plan.md` §2 explains how both tools are used.
