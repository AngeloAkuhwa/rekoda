...
| **Open branches**           | none of Rekoda's own; Dependabot #235 (minor and patch group) is rebased onto this `main` and under review; #236 (Vitest 5) stays on `3158c3c` until #235 merges, then is refreshed and reviewed; #225, #226 and #227 (NestJS 12, one package each) were closed as deferred post-launch work (G-70) |
...
1. Angelo merges #235 when it is reported ready, then #236 after its
   refresh and review (dependency housekeeping before any new launch
   blocker); deletes the leftover GitHub
