...
| ------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| OWN-15 | **`REKODA_EDGE_PROXIES=private_ranges` must not be accepted for a real staging or production deployment** (OD-13, option (a)). In this Docker topology IPv6 visitors and hairpin traffic may reach Caddy through the bridge gateway, which belongs to a private range, so trusting all private ranges makes that gateway a trusted proxy and can let a caller influence the derived client address. The CI smoke must not require a production-unsafe setting merely to simulate Cloudflare | The edge check (`apps/api/src/edge-proxies.ts`, run by `edge-check.ts` before Caddy serves), which refuses `private_ranges` and any range holding the pinned edge gateway; `scripts/check-deploy.mjs`; the deploy smoke's stand-in for Cloudflare (G-75) |
