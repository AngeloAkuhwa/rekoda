...
| **Anthropic** (Claude models) | Merchant message text with supported identifiers tokenised before it is sent; business document and receipt images (raw image bytes) when image AI is enabled | Interpretation of bookkeeping commands; document/receipt extraction; junk-document classification | Anthropic's hosted API (US-based processing) | Processor for AI features; **DPCO to confirm** | `apps/api/src/ai/` (interpreter, `ocr.vision.ts`), flags in `config.ts` (`IMAGE_AI_ENABLED`) |
| **OpenAI** | Voice-note audio (raw audio bytes, transient) when voice transcription is enabled; for high-value dual extraction, the TOKENISED extracted text of the document (never the image - the verifier re-derives the structured command from the same gateway-processed text the primary reasoning saw) | Speech-to-text; second-opinion verification of high-value extractions | OpenAI's hosted API (US-based processing) | Processor for AI features; **DPCO to confirm** | `apps/api/src/ai/stt.openai.ts`; `verifyIfHighValue` in `apps/api/src/ai/interpreter.service.ts` (text-only call path); flags in `config.ts` (`VOICE_TRANSCRIPTION_ENABLED`) |
...
| **Backblaze** | PLANNED, not yet enabled: mirror copies of generated documents (B2). No B2 code or deployment artifact exists in the repository yet; this row records the intended backup design so it is reviewed before it goes live | Off-provider backup of documents (planned) | Backblaze (US) | Backup processor once enabled; **DPCO to confirm** | `backup-restore.md` (production backup procedure, marked not yet enabled) |
...
- Customer identities are AES-256-GCM ciphertext in the vault; no
  provider receives vault plaintext. Model prompts carry meaningless
  tokens in place of supported identifiers, never the identities
  themselves.
