...
| 2 | Bookkeeping conversations | WhatsApp message text, drafts, AI interpretations (PII tokenised before any model) | Merchants, their customers when mentioned | Postgres, tenant-pinned; payload vault for message bodies | Chat history 90 days after account close (`RETENTION.conversationDays`) | **DPCO** |
...
| 6 | Document extraction | Receipt/invoice images, extracted fields | Merchants, counterparties on documents | Image processed via Anthropic (and OpenAI verifier for high-value); evidence media purged 90 days after claim resolution (`RETENTION.evidenceRawDays`) | Per retention schedule rows | **DPCO** |
...
| 9 | Generated documents | Invoice/receipt/statement PDFs and workbooks | Merchants, their customers | R2 primary, B2 mirror; no public URLs | Financial-record retention | **DPCO** |
...
keyed-HMAC blind indexes; PII tokenisation before model prompts;
confirmation gates before financial writes; HMAC-verified webhooks;
key fingerprints checked at boot (A6); encrypted offsite backups with a
CI-run restore drill; append-only audit and journal structures.
