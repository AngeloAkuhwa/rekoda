...
**1. Meta-direct stands.** Twilio passes Meta's fees through _and_ adds its own
...
**2. Bill on messages _processed_, not messages sent.** Meta charges on
...
bounds Rekoda's COGS _by design_ rather than leaving it hostage to how
...
Every outbound message is now COGS, so the conversation design _is_ the margin:
