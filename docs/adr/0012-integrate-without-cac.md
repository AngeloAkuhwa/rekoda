...
> Paystack DVAs as _the_ bank-transfer answer. They are one rung of a ladder, and
...
> proof of legal business identity on its own. So the _original_ Integrate
> design — per-merchant WABA via Embedded Signup — already excluded the same
> merchants, before Paystack was ever considered.

**Precision added 19 Aug 2026 — Meta does not require CAC to _have_ a WABA.**
...
2. ~~Meta may deactivate unverified WhatsApp Business accounts after ~30 days.~~
   **RETIRED 19 Aug 2026 — unsupported.** Meta's own messaging-limits
   ...
   Verification appears only as _one option_ inside a denial alert —
   ...
   _Probable origin of the garbled claim:_ WABAs **can** be restricted immediately
   after creation for Meta policy or integrity reasons, and WhatsApp Manager's
   remediation prompt reads _"Business Verification Needed"_ **even for businesses
...
CAC requirement is a _provider onboarding tier_ policy, not the underlying rule,
...
strictly better _for Rekoda_ than A2, because Rekoda owns the schema instead of
...
| Rung                                         | Mechanism                                                                                                                                                                  | Requires      | Who                    |
| -------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- | ---------------------- |
| **B0 — Open banking account link**           | Merchant links the bank account they **already use** (personal or business) via Mono, with consent. Rekoda reads _incoming credits_ and matches them to expected payments. | BVN + consent | **Everyone**           |
...
the account they already give out, and Rekoda simply gains the ability to _see_
...
for registered merchants" — is replaced by: _every_ Rekoda merchant can reach
...
- **Consent scope is the hard part.** Linking a bank account — especially a
  _personal_ one — exposes the merchant's own spending, not just business
