...
### Platform readiness is three axes, not one status (PR-119)

"An adapter existing in the codebase is not the same as it being available
to a merchant" (spec §18). `provider_capabilities` says which providers the
PLATFORM may offer, per port, and since the owner ruling of 28 August 2026
it says it on three INDEPENDENT axes rather than one blended status:

| Axis                  | Cleared by | Means                                                       |
| --------------------- | ---------- | ------------------------------------------------------------ |
| `technical_support`   | engineering | an adapter exists for this port and passes its sandbox tests |
| `commercial_approval` | commercial  | there is a signed production arrangement                     |
| `compliance_approval` | compliance  | permitted under Rekoda's policy and the applicable regulation |

`production_enabled` is **generated by the database** from all three. That
is the structural form of the rule that a working sandbox must never open
production: passing tests sets exactly one axis, one axis is never enough,
and there is no UPDATE anywhere in this codebase that can shortcut it. Every
axis defaults FALSE, so a provider added tomorrow is closed until three
separate people say otherwise.

They are separate because they fail separately and are cleared by different
people on different timescales, which is the same argument §17.1 makes for a
connection's four statuses. Kuda is the case that proves it: one blended
status showed a single blocker, when in truth its adapter is written and
tested and it is waiting on BOTH commercial terms and regulatory approval. A
closed axis must carry a note saying who or what is holding it, so a refusal
names every outstanding signature rather than the first one — a merchant
told about one blocker who waits for it and is then refused for another has
been told the truth twice and helped neither time.

Standing today: Paystack COLLECT open on all three; Mono and OPay one axis
away, both commercial; Kuda two, commercial and compliance.
