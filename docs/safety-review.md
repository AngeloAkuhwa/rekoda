...
|---|---|---|---|
| A1 | **Paystack** | (a) Is **Pay with Transfer** charged at the DVA rate (1%, cap ₦300) or the local rate (1.5% + ₦100)? (b) Do **per-transaction** transfer accounts carry `subaccount`/`split_code`? (c) Can a platform **delegate sub-merchant KYC to Paystack while staying on the platform-managed flow** — i.e. Paystack's KYC rigour *without* the ₦2M Starter cap? (d) Does the ~1,000 dedicated-account ceiling apply to transient per-transaction accounts (expected: no)? | (a) `/pricing` accuracy · (b) the split is what makes ADR 0013 work — **confirm before building** · (c) a meaningful simplification if real · (d) largely dissolved by ADR 0016 |
...
| A6 | **Meta** (or a BSP) | Current state of business verification for Nigerian merchants; is any non-CAC evidence accepted? | ADR 0012 rung A2 — the WABA upgrade, never the critical path |
