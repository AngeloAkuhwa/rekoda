...
export async function sweepRecurring(
  ...
  /* Claiming a schedule moves next_due_on forward and out of the set, so
   * repeating until a short page drains the whole day's standing orders. */
  const PAGE = 500;
  for (;;) {
    const before = result.raised + result.skipped;
    const due = await recurringRepo.dueSchedules(deps.workerDb, today, PAGE);
